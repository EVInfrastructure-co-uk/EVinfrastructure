...
levi_combined = File.read('_templates/LEVI-combined.html')
scotland = File.read('_templates/scotland.html')
wales = File.read('_templates/wales.html')
...
authorities.select { |a| a['local-authority-type'] == 'COMB' }.each do |authority|
  name = authority['nice-name']
  slug = authority['gov-uk-slug']

  filename = "#{slug}.html"

  content = levi_combined.gsub('{{ authority.name }}',name)
...
authorities.select { |a| a['local-authority-type'] == 'SCO' }.each do |authority|
  ...
  content = scotland.gsub('{{ authority.name }}',name)

  File.write(filename, content)
  puts "Generated #{filename}"
end

authorities.select { |a| a['local-authority-type'] == 'WPA' }.each do |authority|
  name = authority['nice-name']
  slug = authority['gov-uk-slug']

  filename = "#{slug}.html"

  content = wales.gsub('{{ authority.name }}',name)
